...
void plazas(){
    if(analogRead(ir1)<=100){
        analogWrite(rojo1,100);
        digitalWrite(verde1,LOW);
    }
    if(analogRead(ir2)<=100){
        analogWrite(rojo2,100);
        digitalWrite(verde2,LOW);
    }
    if(analogRead(ir3)<=100){
        analogWrite(rojo3,100);
        digitalWrite(verde3,LOW);
        
        if(analogRead(ir1)>100){
            digitalWrite(rojo1,LOW);
            analogWrite(verde1,100);
        }
        if(analogRead(ir2)>100){
            digitalWrite(rojo2,LOW);
            analogWrite(verde2,100);
        }
        if(analogRead(ir3)>100){
    ...
    void semaforo(int pocupadas){
        if(pocupadas==max){digitalWrite(semafororojo,HIGH);digitalWrite(semaforoverde,LOW);}
        if(pocupadas<max){digitalWrite(semafororojo,LOW);digitalWrite(semaforoverde,HIGH);}
    }
